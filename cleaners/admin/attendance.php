	...
	if($result === FALSE) { 
			...
			while ($row=mysql_fetch_array($result)) {				
				...
				while($row1=mysql_fetch_array($result1)){
					$bathid=$row1['id'];
					echo '<div class="foo">';
					$query2="SELECT * FROM attendance where building = '$building' and floor = '$rwid' and id='$bathid' and datee ='$today'";
					...
					for($j=5;$j<52;$j=$j+2)
					{
						if($data[$j]!=0&&($visit[$j]==0)){
		    				...
		    				if($data[$j+1]==0&&$worker!="No Worker Alloted"){
		    					if (date('H')<$ab){		    						
		    						if (floor($ab)==$ab){
		    							echo '<div class = "small" style="width:'.$sc.'%;background:white;border:0.5px solid black">';	
